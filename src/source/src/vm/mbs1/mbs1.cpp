...
#if defined(USE_Z80B_CARD)
	{ _T("Z80BCARD"), DEVTYPE_REG },
#elif defined(USE_MPC_68008)
	{ _T("MPC68008"), DEVTYPE_REG },
#endif
...
void VM::debug_regs_info(uint32_t num, _TCHAR *buffer, size_t buffer_len)
{
	...
	case DNM_FDC5EX:
		// fdc5ex
		if (fdc5ex) fdc5ex->debug_regs_info(c_device_name_list[num].name, buffer, buffer_len);
		break;
	case DNM_CMT:
		// cmt
	...
	case DNM_PSG0:
		// psg
		if (psgst) psgst->debug_regs_info(c_device_name_list[num].name, buffer, buffer_len);
		break;
	case DNM_PSG1:
		// psgex
	...
	case DNM_FMOPN:
		// fm opn
		if (fmopn) fmopn->debug_regs_info(c_device_name_list[num].name, buffer, buffer_len);
		break;
	case DNM_RTC:
		// rtc
	...
	case DNM_Z80B_CARD:
		// Z80B Card
		if (z80b_card) z80b_card->debug_regs_info(c_device_name_list[num].name, buffer, buffer_len);
		break;
#elif defined(USE_MPC_68008)
	case DNM_MPC_68008:
		// MPC-68008
		if (mpc_68008) mpc_68008->debug_regs_info(c_device_name_list[num].name, buffer, buffer_len);
		break;
#endif
	case DNM_EVENT:
		// event

		...
		case EVENT_READY_ON_7:
			{
				...
				if (m_ff00_mask && p_disk[i]->is_ready()) {
					m_ff00_mask = false;
				}
			}
			break;
	...
	if (!pConfig->now_power_off) {
		...
		case FDD_TYPE_58FDD:
			...
			for (int fdcnum = 0; fdcnum < 2; fdcnum++) {
				int i = m_drv_num[fdcnum];
				if (p_disk[i]->is_head_loading() || ((m_drvsel[fdcnum] & 0x0b) == (0x08 | i))) {
					data |= (0x11 << i);
				}
			}
			break;
...
void FLOPPY::save_state(FILEIO *fp)
{
	...
	//
	vm_state_ident.version = Uint16_LE(0x47);
...
void FLOPPY::debug_regs_info(const _TCHAR *title, _TCHAR *buffer, size_t buffer_len)
{
	...
	if (IOPORT_USE_5FDD) {
		...
		if (pConfig->fdd_type == FDD_TYPE_58FDD) {
			...
			UTILITY::sntprintf(buffer, buffer_len, _T(" %X(%s):%02X"), 0, c_reg_names[2], m_drvsel[idx]);
			UTILITY::sntprintf(buffer, buffer_len, _T(" %X(%s):%02X"), 1, c_reg_names[1], m_drvsel[1-idx]);
		} else {
